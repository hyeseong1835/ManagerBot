...
public abstract class Feature
{
    ...
    static async ValueTask<FeatureSetting> LoadFeatureSettingAsync()
    {
        ...
        if (File.Exists(featureSettingPath) == false)
        {
            ...
            Console.WriteLine($"기능 설정 파일이 존재하지 않습니다. 기본 설정을 생성했습니다: {featureSettingPath}");
    ...
    static async ValueTask LoadFeaturesAsync()
    {
        ...
        for (int loadFeatureIndex = 0; loadFeatureIndex < Feature.featureSetting.FeatureLoads.Length; loadFeatureIndex++)
        {
            ...
            if (featureInfo == null)
            {
                Console.WriteLine($"기능 '{loadInfo.Name}'의 정보를 찾을 수 없습니다. 로드하지 않습니다.");
                continue;
            }
            ...
            if (feature == null)
            {
                Console.WriteLine($"기능 '{loadInfo.Name}'을(를) 인스턴스화 할 수 없습니다. 로드하지 않습니다.");
                continue;
            }
            ...
            Console.WriteLine($"기능 '{feature.Info.name}' 인스턴싱.");
        }
        ...
        for (int i = 0; i < featureIndex; i++)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"기능 '{feature.Info.name}' 로드.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"기능 '{feature.Info.name}'을(를) 로드하는 중 오류가 발생했습니다: {ex.Message}");
            }
        }
    ...
    static async ValueTask UnloadFeaturesAsync()
    {
        for (int i = 0; i < loadedFeatures.Length; i++)
        {
            ...
            {
                Console.WriteLine($"기능 '{feature.Info.name}'을(를) 언로드하는 중 오류가 발생했습니다: {ex.Message}");
            }
        }
    }
    ...
    [OnBotInitializeMethod(featureInitializePriority)]
        ...
        where TSetting : class, new()
    {
        ...
        {
            Console.WriteLine($"설정 파일이 존재하지 않습니다. 기본 설정을 생성합니다: {settingPath}");
